...
--except for Bulgarian (\ref{exm:dpBul}) and Macedonian \citep[ in
\citet{wals}]{wals-37}-- don't show an overt realization of the
Determiner position inside the noun phrase (\ref{exm:dpNo})
\citep{harkins1953}. Conversly, Italian (\texttt{ita}) and the other
romance languages explicit that position as a default behaviour, usually
with a free morpheme preceding the noun (\ref{exm:dpPre}) or by
cliticization of the definite article (\ref{exm:dpRon}):
...
\item
  ...
  \item
    ...
    \glt    \enquote{The \emph{dame} is in front to the room}
  \end{enumerate}
\item
  Enclitic
