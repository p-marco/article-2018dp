...
The general idea of this paper is to address the question of how linguistic structures which are not overtly show in L1 can be accessed during the acquisition of a target language which show them. While doing this can be either both purely speculative as grounded on actual data, I will show how the usage of a target collection of linguistic corpora can be useful to test the main hypotheses into narrower facts. The language under observation are indeed a few: on one side `ces` and `slk` as native languages--with no overt position for the articles--on the other `ita` as target language.

The section X provides a theoretical discussion on the top of different theories inside the Generative framework on the status of DP and NP. The section Y is a review of the methods used into the current analysis, in terms of *reproducibility* of the research itself, the policies of data-collection and the explanation of the results. The section W represent a case study made off to test some hypotheses about the categorial differences of DPs during the acquisition.
...
```{r tree1, echo=FALSE, engine='tikz', out.width='70%', fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'svg', fig.cap='Structural comparison between NP-over-DP and DP-over-NP Hypotheses', engine.opts = list(template = "latex/tikz2pdf.tex"), fig.align='center'}
